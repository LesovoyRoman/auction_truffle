...
describe('Auction', () => {
   it('deploy a contract', async () => {
      ...
      assert.equal(manager, auctionManager, 'The manager is the one who launches the smart contract');
   });

   it('auction the item', async () => {
   ...
   it('must bid above latest bid amount', async () => {

        bidder = accounts[3];

        try {

            await auction.methods.bid().send({ from: bidder, value: web.utils.toWei('1', 'ether') })
   ...
   it('bids one more', async () => {

       bidderNew = accounts[4];

       await auction.methods.bid().send({ from: bidderNew, value: web3.utils.toWei('4', 'ether')});

       auctionBid = await auction.methods.latestBid().call();

       assert.equal(auctionBid, web3.utils.toWei('4', 'ether'), 'Latest new bid sent to bid method converted to wei');
